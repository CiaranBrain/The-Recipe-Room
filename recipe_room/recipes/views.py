...
def recipe_list(request):
    """Fetch and display a list of recipes with pagination."""
    recipe_list = Recipe.objects.all().order_by('created_on')

    # Set up pagination for the recipe list (8 per page)
...
def publish_recipe(request):
    """Allow authenticated users to publish a new recipe."""
...
def recipe_edit(request, recipe_id):
    """Allow authenticated users to edit their existing recipes."""
    recipe = get_object_or_404(Recipe, id=recipe_id)
...
def recipe_delete(request, recipe_id):
    """Allow authenticated users to delete their recipes."""
    if request.method == 'POST':
        recipe = get_object_or_404(Recipe, id=recipe_id)
        recipe.delete()
